...
- display popup to confirm request Location Permission, have 2 button
    ...
    - 許可しない  `Denied`  **Button**
        - close popup, call API to get list shop info without localtion data
...
- 1 [![nf](image\jp\mb\101member\Campaigns.png)]() [![nf](image\en\mb\101member\Campaigns.png)]()

- 2 [![nf](image\jp\mb\101member\CampaignDetails.png)]() [![nf](image\en\mb\101member\CampaignDetails.png)]()
...
- required, char(not number), max:80char

##### 名前（カタカナ）  `Kana name`  **Text**

- required, kana string, max:80char
...
- required, max:80char(only include alphabet char, haftsize space)
...
MAX:80CHAR(only include alphabet char, haftsize space)
:   `カード所有者の名前が無効です`
